...
def process(files, output_file, include_other):
    print("\nProcessing files: \n")
        ...
        if data[key]:
            ...
            for i in range(0, top_n_labels[key]):
                try:
                    ...
                    print(f"Failed to index at {i} with list of length {len(c)}")
                    exit(0)
...
def main():
    ...
    for sub_dir in os.listdir("data"):
        ...
        for ref in process(files, output_file=output_file, include_other=include_other):
